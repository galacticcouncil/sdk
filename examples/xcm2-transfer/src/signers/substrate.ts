...
import type { Call } from '@galacticcouncil/xcm2-sdk';
...
export async function signAndSend(
  ...
  const { from, data } = call;

  const extrinsic = api.tx(data);
  ...
  const nextNonce = await api.rpc.system.accountNextIndex(from);
