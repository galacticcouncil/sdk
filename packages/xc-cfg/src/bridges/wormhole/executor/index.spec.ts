...
import { executorBudget } from './index';
...
describe('executorBudget', () => {
  ...
  describe('evm destination', () => {
    it.each([
      ...
      const budget = await executorBudget({ destination: chain });
  ...
  describe('sui destination', () => {
    it('should clear the observed redeem budget', async () => {
      const { gasLimit } = await executorBudget({ destination: sui_chain });
    ...
    it('should budget no value', async () => {
      const { msgValue } = await executorBudget({ destination: sui_chain });
  ...
  describe('solana destination', () => {
    ...
    it('should clear the measured redeem cost when the ata exists', async () => {
      mockSolana(true);
      const { msgValue } = await executorBudget({
        destination: solana,
        asset: sol,
        recipient: RECIPIENT,
      });
    ...
    it('should not charge ata rent when the ata is already open', async () => {
      mockSolana(true);
      const open = await executorBudget({
        destination: solana,
        asset: sol,
        recipient: RECIPIENT,
      });
      mockSolana(false);
      const missing = await executorBudget({
        destination: solana,
        asset: sol,
        recipient: RECIPIENT,
      });
    ...
    it('should assume a missing ata when the recipient is unknown', async () => {
      const { msgValue } = await executorBudget({ destination: solana });
      expect(msgValue).toBeGreaterThan(9_292_040n + 2_039_280n);
    });
