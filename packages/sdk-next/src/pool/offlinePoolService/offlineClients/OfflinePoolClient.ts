...
export abstract class OfflinePoolClient {
  ...
  private assets: Map<number, PersistentAsset> = new Map([]);
  ...
  withAssets(assets: PersistentAsset[]) {
    this.assets = new Map(
      assets.map((asset: PersistentAsset) => [Number(asset.id), asset])
    );
  }

  protected getAssetDynamicFee(assetId: number): AssetDynamicFee | undefined {
