...
class GetIntentOrderTwapBuy extends PapiExecutor {
  async script(client: PolkadotClient) {
    const sdk = await createSdkContext(client);

    const { api, tx } = sdk;

    const order = await api.scheduler.getTwapBuyOrder(10, 0, '1000');
...
new GetIntentOrderTwapBuy(ApiUrl.Lark3, 'Get Intent TWAP BUY order').run();
